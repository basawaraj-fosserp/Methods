...
def get_sold_sle_data(filters):
	...
	condition += f" and dn.status != 'Return Issued'"

	data = frappe.db.sql(f"""
			Select  sle.warehouse, sle.item_code, sum(sle.actual_qty) as actual_qty, sum(dni.amount) as amount, dni.item_group, dni.item_name, sle.voucher_no
			From `tabStock Ledger Entry` as sle
			Left Join `tabDelivery Note Item` as dni ON dni.name = sle.voucher_detail_no and sle.item_code = dni.item_code and sle.warehouse = dni.warehouse
			left Join `tabDelivery Note` as dn ON dn.name = dni.parent and dn.customer != 'Methods Automotive Pvt Ltd - GGN'
