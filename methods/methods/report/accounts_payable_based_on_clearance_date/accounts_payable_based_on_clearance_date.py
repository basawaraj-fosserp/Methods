...
class ReceivablePayableReport:
	...
	def run(self, args):
		...
		for row in self.data:
			if row.voucher_type == "Purchase Invoice":
				is_paid = frappe.db.sql("""
						SELECT 
							CASE 
								WHEN pe.clearance_date IS NULL THEN 'Yes'
								WHEN pe.clearance_date <= CURDATE() THEN 'Paid'
								ELSE 'Yes'
							END AS paid_status,
							pe.clearance_date
	...
	def get_columns(self):
		...
		if self.account_type == "Receivable":
			...
			self.add_column(_("Prepared Payment?"), fieldname="is_actually_paid", fieldtype="Data")
			self.add_column(_("Clearance Date"), fieldname="clearance_date", fieldtype="Data")
